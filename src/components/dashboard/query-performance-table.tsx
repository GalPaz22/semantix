...
export function QueryPerformanceTable({ rows }: { rows: QueryPerformanceRow[] }) {
  if (!rows.length) {
    return <p className="text-sm text-muted">אין כרגע נתוני ביצועים לשאילתות.</p>;
  }
      ...
      <table className="min-w-full border-separate border-spacing-y-2 text-right text-sm">
        <thead className="text-xs uppercase tracking-[0.18em] text-muted">
          <tr>
            ...
            <th className="pb-2">הכנסה שנוצרה</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.query} className="rounded-2xl bg-[#fbf9ff] shadow-[0_8px_24px_rgba(84,55,167,0.05)]">
              ...
              <td className="rounded-l-2xl px-4 py-3 font-semibold text-[#5d44ef]">
