...
export function SourceBreakdownTable({ rows, labels }: SourceBreakdownTableProps) {
  if (!rows.length) {
    return <p className="text-sm text-muted">אין כרגע פירוט מקורות זמין.</p>;
  }
      ...
      <table className="min-w-full border-separate border-spacing-y-2 text-right text-sm">
        <thead className="text-xs uppercase tracking-[0.18em] text-muted">
          <tr>
            ...
            <th className="pb-2">נתח קליקים</th>
            <th className="pb-2">עגלות מיוחסות</th>
            <th className="pb-2">המרת עגלה</th>
          </tr>
        </thead>
        <tbody>
          {orderedRows.map((row) => (
            <tr key={row.source} className="rounded-2xl bg-white shadow-[0_6px_18px_rgba(14,21,29,0.04)]">
              <td className="rounded-r-2xl px-4 py-3 font-semibold text-ink">{labels[row.source]}</td>
              ...
              <td className="px-4 py-3 text-muted">{row.attributedCarts.toLocaleString("he-IL")}</td>
