...
export async function processWooHybrid({ wooUrl, wooKey, wooSecret, userEmail, categories, userTypes, softCategories, dbName }) {
  ...
  try {
    ...
    while (hasMorePages) {
      // Retry each page so a transient error doesn't silently truncate the catalog.
      let response;
      let lastErr;
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          response = await api.get("products", { per_page: 100, page: page });
          break;
        } catch (err) {
          lastErr = err;
      ...
      // Don't trust x-wp-totalpages alone (caching/security plugins strip it);
      // keep paging while we get full pages, with totalPages as an extra cap.
    ...
    await addLog(`🏪 Found ${allProducts.length} products (no stock/status filtering).`);

    // 1. Initial Basic Upsert — in-stock only
    await addLog(`💾 Saving/updating basic data for ${inStockProducts.length} in-stock products...`);
    ...
    const productsToProcessList = inStockProducts.filter(prod => !existingProductIds.has(prod.id));

    if (productsToProcessList.length === 0) {
      await addLog(`✨ All products already have embeddings or are out of stock - skipping AI enrichment.`);
