...
async function processRSSProducts({ feedUrl, userEmail, categories, userTypes, softCategories, colors, dbName }) {
  ...
  try {
    ...
    const productsToProcess = allProducts.filter(p => !existingProductIds.has(p.id));

    await addLog(`✅ ${existingProductIds.size} already have embeddings (will skip)`);
    await addLog(`🔄 ${productsToProcess.length} products need AI processing`);

    // ── 3. Save / upsert basic data for all products ──────────────────────────
    await addLog(`💾 Saving basic data for all ${allProducts.length} products...`);
