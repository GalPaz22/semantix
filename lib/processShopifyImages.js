...
import { parsePrice, parseHtmlToPlainText, isValidImageUrl, gidToNumeric } from './shared/utils.js';
...
function processShopifyVariants(product) {
  ...
  const variants = product.variants.edges.map(({ node }) => {
    ...
    const variantPrice = parsePrice(node.price, false);
    const variantCompareAtPrice = parsePrice(node.compareAtPrice, false);
...
export default async function processShopifyImages({ shopifyDomain, shopifyToken, shopifyClientId, shopifyClientSecret, dbName, categories, userTypes, softCategories, colors, context }) {
  console.log('🔍 Starting processShopifyImages with:', { shopifyDomain, dbName, context });

  // Public products.json works with custom Shopify storefront domains and myshopify.com domains.
  const fullDomain = normalizeShopifyStorefrontDomain(shopifyDomain);

  console.log('🔧 Domain processing:', { original: shopifyDomain, final: fullDomain });
  ...
  // Step 1: Save in-stock products only with basic data and stock status
  console.log('💾 Step 1: Saving in-stock products to database...');
  for (const product of inStockProducts) {
    ...
    const basicProductData = {
      ...
      price: parsePrice(product.priceRange?.minVariantPrice?.amount, false),
  ...
  console.log(`✅ Saved ${inStockProducts.length} in-stock products to database (${outOfStockProducts.length} out-of-stock skipped)`);
  ...
  console.log(`📊 AI Processing Summary (In-Stock Only):`);
  ...
  await Promise.all(
    productsWithoutEmbeddings.map(prod =>
      limit(async () => {
        ...
        const price = parsePrice(rawPrice, false);
