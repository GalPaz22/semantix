...
export default async function processShopifyImages({ shopifyDomain, shopifyToken, dbName, categories, userTypes, softCategories }) {
  ...
  while (hasNextPage) {
    const variables = { first: 1, after: cursor };
  ...
  await Promise.all(
    products.map(prod =>
      limit(async () => {
        console.log(`🔍 Processing product: ${prod.title} (${done + 1}/${products.length})`);
        ...
        const progress = Math.round((done / products.length) * 100);
        await statusCol.updateOne(
          { dbName },
          { $set: { done, progress } }
        );
        
        console.log(`✅ Completed ${prod.title} (${done}/${products.length} - ${progress}%)`);
      })
    )
  );
