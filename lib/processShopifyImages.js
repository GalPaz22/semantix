...
4. **Colors** [Product colors - choose ALL colors that apply based on images, description, and variants]
   Available: [${colorList}]` : ''}

CLASSIFICATION RULES:
✓ Use the IMAGES to identify visual characteristics (colors, style, materials, patterns)
✓ Use VARIANTS to confirm available options (if variants show "red" option, include the matching color)
✓ Be generous with soft categories - include all relevant search terms customers might use
✓ For categories and types, be more selective - only clear, confident matches${colorsArray.length > 0 ? '\n✓ For colors, analyze ALL images and variants to identify every available color' : ''}
...
{"category": ["Category1", "Category2"], "type": ["Type1", "Type2"], "softCategory": ["Tag1", "Tag2", "Tag3"]${colorsArray.length > 0 ? ', "colors": ["Color1", "Color2"]' : ''}}`
        },
