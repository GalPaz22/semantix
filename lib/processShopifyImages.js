...
import { resolveShopifyToken } from './shared/shopifyAuth.js';
...
export default async function processShopifyImages({ shopifyDomain, shopifyToken, shopifyClientId, shopifyClientSecret, dbName, categories, userTypes, softCategories, colors, context }) {
  ...
  // Resolve access token (2026 client credentials OR legacy static token)
  let accessToken;
  try {
    accessToken = await resolveShopifyToken(fullDomain, shopifyToken, shopifyClientId, shopifyClientSecret);
  } catch (err) {
    console.error('❌ Failed to obtain Shopify access token:', err.message);
    return [];
  }
