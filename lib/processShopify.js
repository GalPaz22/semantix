...
export default async function processShopifyDescriptions({ shopifyDomain, shopifyToken, dbName, categories, type: userTypes, softCategories, colors }) {
  ...
  const products = allProducts.filter(prod => {
    ...
    // Determine stock status based on variant inventory
    ...
    // If product doesn't track inventory, assume in stock
  ...
  await log(`🏪 Found ${allProducts.length} total products. Filtering for in-stock and active products...`);
  await log(`🔄 ${products.length} products match in-stock/active criteria and will be processed.`);
  ...
  await Promise.all(products.map(prod => limit(async () => {
    ...
    if (!translatedDescription) {
      await log(`⚠️ Skipping ${prod.title} - translation failed (DB: ${dbName})`);
      done += 1;
