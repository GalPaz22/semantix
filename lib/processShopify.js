...
export default async function processShopifyDescriptions({ shopifyDomain, shopifyToken, dbName, categories, type: userTypes, softCategories }) {
  ...
  await Promise.all(products.map(prod => limit(async () => {
    ...
    // 2) Translate and enhance the description (preserve full content)
    const translatedDescription = await translateDescription(cleanedDescription);
    ...
    // 3) Build metadata enrichment AFTER translation to ensure it's included
    let metadataAppend = '';
    if (prod.productType) {
    ...
    // 4) Create final enriched description with translated text + guaranteed metadata
