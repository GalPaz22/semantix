...
export default async function processShopifyDescriptions({ shopifyDomain, shopifyToken, shopifyClientId, shopifyClientSecret, dbName, categories, type: userTypes, softCategories, colors }) {
  ...
  // Resolve access token — supports both 2026 client credentials and legacy static token
  let accessToken;
  try {
    accessToken = await resolveShopifyToken(fullDomain, shopifyToken, shopifyClientId, shopifyClientSecret);
  } catch (err) {
    await log(`❌ Failed to obtain Shopify access token: ${err.message}`);
    await statusCol.updateOne({ dbName }, { $set: { state: "error", error: err.message } });
    await client.close();
    return;
  }

  const clientGraphQL = new GraphQLClient(`https://${fullDomain}/admin/api/2024-10/graphql.json`, {
    headers: {
  ...
  // For client credentials, tokens expire after 24 h — refresh mid-sync if needed
  const refreshToken = async () => {
    if (shopifyClientId && shopifyClientSecret) {
      accessToken = await resolveShopifyToken(fullDomain, null, shopifyClientId, shopifyClientSecret);
      clientGraphQL.setHeader('X-Shopify-Access-Token', accessToken);
    }
  };
