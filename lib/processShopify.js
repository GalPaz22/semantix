    ...
    const messages = [
      {
        role: "user",
        parts: [{
          text: `Translate the following text to English and enhance it for e-commerce search. Make it keyword-rich and factual:\n\n${description}`
        }]
      }
    ];
...
export default async function processShopifyDescriptions({ shopifyDomain, shopifyToken, dbName, categories, type: userTypes, softCategories }) {
  ...
  await Promise.all(products.map(prod => limit(async () => {
    ...
    // 3) Create enriched original description with metadata
    const enrichedOriginalDescription = `${cleanedDescription}\n\n${metadataAppend}`.trim();
    
    // 4) Translate and enhance the entire enriched description
