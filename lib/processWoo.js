...
async function processWooProducts({ wooUrl, wooKey, wooSecret, userEmail, categories, userTypes, softCategories, colors, dbName }) {
  ...
  try {
    ...
    while (hasMorePages) {
      try {
        // Note: _embed removed — it bloats every response with unnecessary nested data
        const response = await api.get("products", { per_page: 100, page, status: 'publish' });
    ...
    await addLog(`📦 ${allProducts.length} total — ${inStockProducts.length} in-stock, ${allProducts.length - inStockProducts.length} out-of-stock (skipped)`);
    ...
    // Save basic data for all in-stock products (fast, no AI)
    await addLog(`💾 Saving basic data for ${inStockProducts.length} in-stock products...`);
      ...
      try {
        ...
        await collection.updateOne(
          { id: product.id },
          {
            ...
            $setOnInsert: { embedding: null, category: [], type: [], softCategory: [], description1: null, processedAt: null }
          },
          { upsert: true }
        );
      } catch (error) {
    ...
    if (productsToProcess.length === 0) {
      ...
      await addLog(`✅ WooCommerce processing complete. ${inStockProducts.length} in-stock products saved.`);
      await client.close();
      return logs;
    }
