...
async function processWooProducts({ wooUrl, wooKey, wooSecret, userEmail, categories, userTypes, softCategories, colors, dbName }) {
  ...
  try {
    ...
    await addLog(`📦 Found ${allProducts.length} total products — ${inStockProducts.length} in-stock, ${allProducts.length - inStockProducts.length} out-of-stock (skipped)`);
    ...
    await addLog(`🔄 ${productsToProcess.length} in-stock products need processing`);

    // Save basic data for in-stock products only
