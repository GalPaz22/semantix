...
5.  Do NOT invent new categories, types, or soft categories.
...
async function reprocessProducts({ dbName, categories, userTypes, softCategories }) {
  const logs = [];
  ...
  */
  ...
  const productsToProcess = await collection.find({
    ...
    $and: [
      // Only process products that are explicitly in stock
      {
        $or: [
          { stockStatus: "instock" },
  ...
  await addLog(`🔄 Processing ${totalToProcess} in-stock products for enrichment`);
  if (totalToProcess === 0) {
    await addLog("✅ Nothing to do. All in-stock products with embeddings already have category/type data (out-of-stock products skipped).");
  }
    ...
    try {
      ...
      if (!description1 || description1.trim() === "") {
        ...
        if (isImageSyncMode) {
          ...
          // Use existing description1 and images to classify
          await addLog(`🔍 Vision-classifying with soft categories: ${safeSoftCategories.join(', ') || 'None'}`);
          const classificationResult = await classifyWithImages(description1, name, categories, userTypes, safeSoftCategories, images);
          ...
          // Update only category, type, and timestamp
          await collection.updateOne(
            { _id: product._id },
            { $set: { 
          ...
          await addLog(`📝 Using text-only classification for: ${name}`);
          await addLog(`🔍 Text-classifying with soft categories: ${safeSoftCategories.join(', ') || 'None'}`);
          const classificationResult = await classifyCategoryAndTypeWithGemini(enrichedDescription, name, categories, userTypes, safeSoftCategories);
          ...
          await collection.updateOne(
            ...
            { $set: { 
                ...
                url: product.url || product.permalink || product.link || null,
                image: product.image || (product.images && product.images.length > 0 ? product.images[0].src : null),
                // Convert price fields to numbers if they exist and are strings
                ...(product.price && { price: Number(product.price) || 0 }),
                ...(product.regular_price && { regular_price: Number(product.regular_price) || 0 }),
                ...(product.sale_price && { sale_price: Number(product.sale_price) || 0 }),
                categoryTypeProcessedAt: new Date() 
            } }
          );
