...
/**
 * Convert a product name to a useful Latin-script search name.
 * Proper names are transliterated while generic product words are translated.
 */
async function translateProductNameToEnglish(name, callGemini) {
  ...
  try {
    const result = await callGemini({
      contents: [{
        role: "user",
        parts: [{
          text: `Convert this e-commerce product name into a natural Latin-script name for search.

Rules:
1. Transliterate Hebrew proper names, local brands, wineries, people, and model names phonetically. Do not translate their literal meaning.
   Example: "עגור לים" -> "Agur Layam".
2. Translate generic product words into English.
   Examples: "בירה" -> "Beer", "יין אדום" -> "Red Wine", "ג'ין" or "ג׳ין" -> "Gin".
3. Restore well-known international brand or product spellings instead of transliterating them mechanically.
   Examples: "אפרול" -> "Aperol", "קמפרי" -> "Campari", "ג'ק דניאלס" -> "Jack Daniel's".
4. For mixed names, transliterate the proper-name portion and translate only the generic/descriptive portion.
5. Preserve existing Latin text, model numbers, vintages, sizes, quantities, percentages, and factual details.
6. Use Latin characters for Hebrew words. Do not include the original Hebrew.
7. If the name is already fully in Latin script, return it unchanged.
8. Return only the converted product name, with no label, quotes, explanation, or commentary.
...
${originalName}`
    ...
    return (result?.trim() || originalName).replace(/\s+/g, ' ');
  } catch (error) {
    console.warn("Product name translation failed:", error);
    return originalName;
  }
}

function appendEnglishName(originalName, translatedName) {
  const cleanOriginal = typeof originalName === 'string' ? originalName.trim() : '';
  const cleanTranslation = typeof translatedName === 'string' ? translatedName.trim() : '';

  if (!cleanOriginal) return cleanTranslation;
  if (!cleanTranslation || cleanOriginal.localeCompare(cleanTranslation, undefined, { sensitivity: 'accent' }) === 0) {
    return cleanOriginal;
  }

  return `${cleanOriginal} | ${cleanTranslation}`;
}
  ...
  try {
    ...
    const query = {
      $and: [
        ...
        { $or: [{ stockStatus: "instock" }, { stock_status: "instock" }] }
      ]
    };
...
async function reprocessProducts({
  ...
  if (options.reprocessAll) {
    options = {
      ...
      appendTranslatedNameToName: true
    };
  }
  ...
  await log(`${options.appendTranslatedNameToName ? '✅' : '❌'} Append English Name to Product Name`);
  ...
  if (options.appendTranslatedNameToName && !incrementalMode && !onlyUnprocessed && !missingSoftCategoryOnly) {
    classificationConditions = null;
    await log("📝 Translated-name mode: Processing all eligible in-stock products");
  }
  ...
  for (const product of productsToProcess) {
    ...
    try {
      ...
      if (!originalDescription || originalDescription.trim() === "") {
        ...
        let enrichedDescription = options.reprocessDescriptions
          ? (product.description || originalDescription)
          : (product.description1 || product.description || originalDescription);
        ...
        if (options.appendTranslatedNameToName) {
          try {
            const originalName = typeof product.originalName === 'string' && product.originalName.trim()
              ? product.originalName.trim()
              : (typeof name === 'string' ? name.trim() : '');
            const translatedName = await translateProductNameToEnglish(originalName, callGeminiFn);
            if (translatedName) {
              updateData.originalName = originalName;
              updateData.name = appendEnglishName(originalName, translatedName);
              await log(`✅ Updated product name: ${updateData.name}`);
            }
          } catch (nameTranslationError) {
            await log(`❌ Product name translation error: ${nameTranslationError.message}`);
          }
        }
...
function parseCliArgs(args) {
  ...
  args.forEach(arg => {
    ...
    } else if (arg === '--append-translated-name') {
      options.appendTranslatedNameToName = true;
