...
async function reprocessProducts({ dbName, categories, userTypes, softCategories, targetCategory = null, missingSoftCategoryOnly = false }) {
  ...
  */
  ...
  if (isImageSyncMode) {
    ...
    if (missingSoftCategoryOnly) {
      ...
      classificationConditions = {
        $and: [
          { category: { $exists: true } }, // Must have category field
  ...
  if (missingSoftCategoryOnly) {
    ...
    const productsWithSoftCategory = await collection.countDocuments({
      embedding: { $exists: true, $ne: null },
      stockStatus: "instock", 
      softCategory: { $exists: true }
    });
    
    await addLog(`🔍 Debug: Total in-stock products with embeddings: ${totalProducts}`);
    ...
    await addLog(`🔍 Debug: Products with softCategory field: ${productsWithSoftCategory}`);
    
    // Let's also get a sample product to see its structure
    ...
    if (sampleProduct) {
      await addLog(`🔍 Debug: Sample product structure - has category: ${!!sampleProduct.category}, has softCategory: ${!!sampleProduct.softCategory}`);
      await addLog(`🔍 Debug: Sample category value: ${JSON.stringify(sampleProduct.category)}`);
      await addLog(`🔍 Debug: Sample softCategory value: ${JSON.stringify(sampleProduct.softCategory)}`);
    }
  }
