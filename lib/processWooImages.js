...
export default async function processWooImages({ wooUrl, wooKey, wooSecret, dbName, categories, type: userTypes, softCategories, colors, context }) {
  ...
  const inStockProducts = allProducts.filter(p => p.stock_status === 'instock');
  await log(`📦 Found ${allProducts.length} total products — ${inStockProducts.length} in-stock, ${allProducts.length - inStockProducts.length} out-of-stock (skipped).`);
