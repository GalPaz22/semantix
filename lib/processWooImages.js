...
import { applyWooDefaultBoosts, withWooPopularityData } from './shared/wooPublicProducts.js';
...
export default async function processWooImages({ wooUrl, wooKey, wooSecret, dbName, categories, type: userTypes, softCategories, colors, context }) {
  ...
  // Filter to in-stock products only — out-of-stock products are never saved or enriched
  // Ingest all fetched products (no in-stock / publish filtering).
  const inStockProducts = allProducts;
  await log(`📦 Found ${allProducts.length} total products (no stock/status filtering).`);

  await statusCol.updateOne({ dbName }, { $set: { total: inStockProducts.length } });

  // 1. Initial Basic Upsert — in-stock only
  await log(`💾 Saving/updating basic data for ${inStockProducts.length} in-stock products...`);
  ...
  const popularityStats = await applyWooDefaultBoosts(productsCol, inStockProducts);
  await log(`⭐ Woo popularity: ${popularityStats.detected} detected, ${popularityStats.boosted} received default boost level 1`);
  ...
  const productsToEnrich = inStockProducts.filter(prod => !existingProductIds.has(prod.id));

  if (productsToEnrich.length === 0) {
    await log(`✨ All products already have embeddings or are out of stock - skipping AI enrichment.`);
  ...
  await statusCol.updateOne(
    { dbName }, { $set: { state: "done", finishedAt: new Date(), done: inStockProducts.length } }
  );

  await log(`🏁 Done! Processed ${inStockProducts.length} in-stock products. (DB: ${dbName})`);
